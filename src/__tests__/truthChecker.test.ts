describe("checking for truth", () => {
  test("are the littleNumbers < bigNumbers?", () => {
    ...
    let bigNumbers: Array<number> = [100, 200, 300, 400, 500];
  ...
  test("wordToCheck is a palindrome, case and everything", () => {
    let wordToCheck = "amanaplanacanalpanama"; // yeah, replace the value to pass the test.
    expect(wordToCheck.split("").reverse().join("")).toBe(wordToCheck);
  });
  test("true", () => {
    let val = 3 < 4; //set this to a passing value
    expect(val).toBeTruthy();
  });
  test("there is a sequence to this test", () => {
    let values: Array<any> = ["lament", true, 3, null]; // set values here - 
                                                        // the array will take any type or combo of types.
